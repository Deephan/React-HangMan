...
function GridCell(props){
  return (<rect x={props.x} y={props.y} rx="3" ry="3" width="20" height="20" stroke="LightSlateGrey" fill="transparent" />)
}

function GridCellElement(props){
  let _color = "OrangeRed"
  if(props.letter === '_') _color = "DarkRed"
  return (<text x={props.x} y={props.y} font-family="Verdana" font-size="15" fill={_color}>{props.letter}</text>)
}
...
function Grid(props){
  ...
  return(
    <svg xmlns="http://www.w3.org/2000/svg" width="850" height="100" viewBox="0 0 300 50">
      {word.split('').map((ele, index) => {
        ...
        return(
          <g>
            <GridCell x={GC_X} y={GC_Y}/>
            <GridCellElement x={GCE_X} y={GCE_Y} letter={ele}/>
          </g>
        )
    })}
    </svg>
...
function QuestionCard(props){
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="450" height="100">
      ...
      <text x="170" y="35" font-family="Trebuchet MS" font-size="30" fill="Teal">
        {props.question}
      </text>
      </svg>
  )
...
function DebugTallyCard(props){
  ...
  return (
    <h4>
    Unique letters: {Array.from(props.uniq).join(",")}
    <br/>
    Tally letters: {Array.from(props.tally).join(",")}
    <br/>
    Answer: {props.answer}
    <br/>
    </h4>
...
function LetterTallyCard(props){
  ...
  return (
    <div>
      <DebugTallyCard debug={props.debug} uniq={props.uniq} tally={props.tally}/>
    </div>
  )
}
...
class HangMan extends React.Component {
  ...
  getUniqueLetters(word){
    ...
    this.setState({uniqueLetters: uniq})
  }
  ...
  render() {
    ...
    return (
      <form onSubmit={this.handleSubmit}>
          <QuestionCard question={triviaBoard[index].q}/>
          <Grid 
            word={this.state.answer.join('')} 
            cells={triviaBoard[index].a.length}/>
          <LetterTallyCard 
            debug={_debug}
            uniq={this.state.uniqueLetters} 
            tally={this.state.tallyLetters} 
